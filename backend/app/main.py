...
def coach(
    ...
    active_plan = services.get_current_plan(db, user.user_id)
    current_plan_dict = services.serialize_plan(active_plan) if active_plan else None
    reply, context_type, execution_debug = services.build_coach_reply(
        hydrated, payload.message, recent, logs, current_plan=current_plan_dict
    )
...
def coach_stream(
    ...
    current_plan_dict = services.serialize_plan(active_plan) if active_plan else None
    reply, context_type, execution_debug = services.build_coach_reply(
        hydrated, message, recent, logs, current_plan=current_plan_dict
    )
    runtime_info = get_runtime().info()
    interaction = models.AIInteraction(
        user_id=user.user_id,
...
@app.post("/cloud/set-endpoint")
def set_cloud_endpoint(payload: dict):
    """Update the ngrok/cloud inference URL at runtime without restarting.
    Body: {"url": "https://xxxx-xx-xx.ngrok-free.app"}
    """
    import os
    url = (payload.get("url") or "").strip().rstrip("/")
    if not url:
        raise HTTPException(status_code=400, detail="url is required")
    os.environ["FITSENSE_CLOUDRUN_URL"] = url
    runtime = get_runtime()
    runtime.refresh_configuration(force=True)
    return {
        "message": "Cloud endpoint updated",
        "cloud_predict_url": runtime.cloud_predict_url,
        "can_use_cloud": runtime._can_use_cloud(),
    }


@app.get("/cloud/status")
def cloud_status():
    """Check what cloud endpoint is configured and whether it's reachable."""
    runtime = get_runtime()
    ...
    return {
        ...
        "last_load_error": runtime._load_error,
    }
