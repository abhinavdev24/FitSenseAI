...
class StudentLLMRuntime:
    ...
    def _strip_think_tags(self, text: str) -> str:
        """Remove <think>...</think> chain-of-thought blocks the model sometimes emits."""
        cleaned = re.sub(r"<think>.*?</think>", "", text, flags=re.DOTALL | re.IGNORECASE)
        return cleaned.strip()

    def generate_coach_text(self, *, user_message: str) -> str | None:
        cloud_result = self._call_cloud(task="coach_text", system_prompt=SYSTEM_COACH_PROMPT, user_message=user_message, max_new_tokens=512)
        if cloud_result is not None:
            text = cloud_result.get("text") or cloud_result.get("raw_text")
            if isinstance(text, str) and text.strip():
                cleaned = self._strip_think_tags(text)
                print(f"[llm-output] coach text:\n{cleaned}")
                return cleaned or None
        text = self.generate_text(system_prompt=SYSTEM_COACH_PROMPT, user_message=user_message, max_new_tokens=512)
        if text is None:
            return None
        cleaned = self._strip_think_tags(text)
        if not cleaned:
